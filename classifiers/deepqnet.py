...
class DeepQNet():
		...
		def create_fc_model(self, input_size, layers):
			...
			self.prediction = self.create_fc_layer(last_layer, [last_layer_size, self.n_actions], 'readout_layer', activation=None)

		def create_train(self):
			with tf.name_scope('training'):
				action = tf.reduce_sum(tf.mul(self.prediction, self.a), reduction_indices=1)
		...
		def train_batch(self, batch, verbose=False):
			...
			self.train.run(session=self.sess, feed_dict={self.x: state, self.y: q_value, self.a: action, self.lr: lrate, self.keep_prob: self.training_dropout})
